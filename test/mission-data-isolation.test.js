...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    ...
    const eventToAdd = {
      temperature: 20,
      reason: `Thermostat-failed-unique:${Date.now()}`, // This must be unique
      color: 'Green',
      weight: 80,
      status: 'active',
    ...
    expect(receivedResponse.body.id).toBeDefined();
    // 💡 TIP: Check not only the HTTP status bot also the body
    expect(Object.entries(receivedResponse.body).length).toBeGreaterThan(0);
  });
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    ...
    expect(Object.entries(firstResponse.body).length).toBeGreaterThan(0);
    expect(secondResponse.status).toBe(409);
    expect(secondResponse.body.id).not.toBeDefined();
    expect(Object.entries(secondResponse.body).length).toBe(0);
  });
  ...
  test('When querying for event by id, Then the right event is being returned', async () => {
    ...
    // ACT
    const firstResponse = await request(expressApp).post('/sensor-events').send(eventToAdd);
    const localResponse = await request(expressApp).get(`/sensor-events/${firstResponse.body.id}`,);

    expect(localResponse.status).toBe(200);
    expect(Object.entries(localResponse.body).length).toBeGreaterThan(0);
  });
