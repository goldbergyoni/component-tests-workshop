...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    // Arrange
    const uniqueReason = getShortUnique();
    const eventToAdd = {
      category: 'Home equipment',
      temperature: 20,
      reason: uniqueReason, // This must be unique
      color: 'Green',
      weight: 80,
      status: 'active',
    };
    ...
    // 💡 TIP: Check not only the HTTP status but also the body
  });
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    ...
    // Assert
    expect(receivedResponse).toMatchObject({ status: 409 });
  ...
  test('When querying for event by id, Then the right event is being returned', async () => {
    ...
    // Act
    const receivedResponse = await request(expressApp).get("/sensor-events/" + postResponse.body.id ).send();
  ...
  test('When temperature is not specified,  the event is NOT saved to the DB', async () => {
    // Arrange
    const uniqueReason = getShortUnique();
    const eventToAdd = {
    ...
    // Act
    const receivedResponse = await request(expressApp).post("/sensor-events").send(eventToAdd);
    const receivedGetResponse = await request(expressApp).get("/sensor-events").send();

    // Assert
    expect(receivedResponse.status).toBe(400);
    expect(receivedGetResponse.body.find(x => x.reason === uniqueReason)).toBeFalsy();
  });
