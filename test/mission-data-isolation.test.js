...
beforeAll(async () => {
  ...
  expressApp2 = await startWebServer();
});
...
describe('Sensors test', () => {
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    ...
    const firstResult = await request(expressApp).post('/sensor-events').send(eventToAdd);
  ...
  test('When adding multiple events, then all of them appear in the result', async () => {
    ...
    const resp1 = await request(expressApp).post('/sensor-events').send(eventToAdd1);
    const resp2 = await request(expressApp).post('/sensor-events').send(eventToAdd2);
    ...
    expect(receivedResponse.body).toEqual(
        expect.arrayContaining([
          expect.objectContaining({id: resp1.body.id}),
          expect.objectContaining({id: resp2.body.id}),
        ]),
    );
  });
  ...
  // test('Delete event and validate', async () => {
  //   // Arrange
  //   const eventToAdd = {
  //     category: 'Home equipment',
  //     temperature: 20,
  ...
  //
  //   // Assert
  //   const responseForEventDeleted = await request(expressApp).get(
  //       `/sensor-events/${deletedId}`,
  //   );
  //   expect(responseForEventDeleted.body).toBeNull();
  // });
  ...
  //   // Act
  //   const receivedResponse = await request(expressApp2).get(`/sensor-events/${id}`);
  //   expect(receivedResponse.status).toEqual(404);
  //
  // });
