...
describe('Sensors test', () => {
  ...
  test('When emitting a new event and the notification service replies with 500 error, then the added event was still saved successfully', async () => {
    ...
    const receivedText = JSON.parse(receivedResponse.text);

    const savedResponse = await request(expressApp)
      .get(`/sensor-events/${receivedText.id}`);

    const savedText = JSON.parse(savedResponse.text);
    ...
    expect(savedText.id).toEqual(receivedText.id);
  });
});
