...
describe('Sensors test', () => {
  ...
  test('When temperature is above 50, then the right notification should be sent', async () => {
   ...
   const nockNotification = nock('http://localhost').post(
    ...
    // Assert
    expect(nockNotification.isDone()).toBe(true)
    expect(notificationPayload).toMatchObject({ title: 'Something critical happened', id: expect.any(Number) });
  ...
  test('When emitting a new event and the notification service replies with 500 error, then the added event was still saved successfully', async () => {
    ...
    // Act
    const { body: { id }} = await request(expressApp)
    .post('/sensor-events')
    .send(eventToAdd);
    
    const getNewEvent = await request(expressApp).get(`/sensor-events/${id}`);
    // Assert
    // Assert
    expect(getNewEvent.status).toEqual(200);
    expect(getNewEvent.body.id).toEqual(id);
...
test('When emitting event and the notification service fails once, then a' +
  ' notification is still being retried and sent successfully', async () => {
  // Arrange
  const eventToAdd = getSensorEvent({
    temperature: 80, //💡 TIP: We need high temperature to trigger notification
    notificationCategory: getShortUnique(), //💡 TIP: Unique category will lead to unique notification URL. This helps in overriding the nock
