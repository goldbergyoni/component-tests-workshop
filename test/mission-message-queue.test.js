...
test('Whenever a new sensor event arrives, then its retrievable', async () => {
  ...
  const response = await request(expressApp).get(`/sensor-events/${eventToPublish.category}/temperature`);
...
test('Whenever an invalid events arrives, then its being rejected', async () => {
  // Arrange
  const invalidEventToPublish = testHelpers.getSensorEvent({ temperature: null });
  ...
  // Act
  await messageQueueClient.publish('sensor.events', 'events.new', invalidEventToPublish);
...
test('Whenever an invalid events arrives, then its being rejected and the event was not saved to DB', async () => {
  // Arrange
  const invalidEventToPublish = testHelpers.getSensorEvent({ temperature: null });
  ...
  const response = await request(expressApp).get(`/sensor-events/${invalidEventToPublish.category}/temperature`);
  expect(response).toMatchObject({
    status: 200,
    body: [],
  });
...
test('When a new event is posted via API, then a message is put in the analytics queue', async () => {
  ...
  const spyPublish = sinon.spy(MessageQueueClient.prototype, 'publish');
  ...
  expect(spyPublish.called).toBe(true);
  expect(spyPublish.lastCall.args).toMatchObject([
    'analytics.events',
    'analytics.new',
    eventToAdd,
  ]);
