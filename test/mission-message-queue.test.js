...
test('Whenever a new sensor event arrives, then its retrievable', async () => {
  ...
  const anEvent = await request(expressApp).get(`/sensor-events/${eventToPublish.category}/category`);
  expect(anEvent).toMatchObject({ status: 200, body: [eventToPublish], });
});
...
test('Whenever an invalid events arrives, then its being rejected', async () => {
  ...
  const messageQueueClient = await testHelpers.startMQSubscriber('fake', 'events.new');
  await messageQueueClient.publish('sensor.events', 'events.new', invalidEvent);
  await messageQueueClient.waitFor('nack', 1);
  const anEvent = await request(expressApp).get(`/sensor-events/${invalidEvent.category}/category`);
  expect(anEvent).toMatchObject({ body: [] });
});
