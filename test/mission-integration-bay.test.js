...
beforeEach(() => {
  nock.cleanAll();
  nock('https://google.com').get('/').reply(200, { success: true });
});
...
describe('Sensors test', () => {
  ...
  test('When temperature is above 50, then the right notification should be sent', async () => {
    ...
    expect(notificationPayload).toMatchObject({id: expect.any(Number), title: expect.any(String)})
  });
  ...
  test('When emitting a new event and the notification service replies with 500 error, then the added event was still saved successfully', async () => {
    ...
    const addedEvent = await request(expressApp)
    .get(`/sensor-events/${receivedResponse.body.id}`)
    expect(addedEvent.body).toMatchObject({...eventToAdd})
  });
...
// ✅ TASK: Write the following test below
test('When emitting a new event and the notification service replies with 500 error after a short delay, then the added event was still saved successfully', async () => {
  // Arrange
  const eventToAdd = getSensorEvent({
    temperature: 80, //💡 TIP: We need high temperature to trigger notification
    notificationCategory: getShortUnique(), //💡 TIP: Unique category will lead to unique notification URL. This helps in overriding the nock
  ...
  nock('http://localhost').post(`/notification/${eventToAdd.notificationCategory}`
).delay(Number(process.env.HTTP_TIMEOUT) + 1000).reply(500);
...
test('When emitting a new event and the notification service replies with 500 error after a delay, then the added event was still saved successfully', async () => {
  // Arrange
  const eventToAdd = getSensorEvent({
    temperature: 80, //💡 TIP: We need high temperature to trigger notification
    notificationCategory: getShortUnique(), //💡 TIP: Unique category will lead to unique notification URL. This helps in overriding the nock
  ...
  nock('http://localhost').post(`/notification/${eventToAdd.notificationCategory}`
).delay(Number(timeInMillisecond)).reply(500);
...
test('When emitting event and the notification service fails once, then a notification is still being retried and sent successfully', async () => {
  ...
  let notificationPayloadWithError, notificationPayloadWithSuccess;
    nock('http://localhost').post(`/notification/${eventToAdd.notificationCategory}`,
    (payload) => (notificationPayloadWithError = payload),
  ).times(1).reply(500);
    nock('http://localhost').post(`/notification/${eventToAdd.notificationCategory}`,
    (payload) => (notificationPayloadWithSuccess = payload),
  ).reply(200);
  ...
  // Assert
  // expect(notificationPayloadWithError).toMatchObject({id: expect.any(Number), title: expect.any(String)})
  expect(notificationPayloadWithSuccess).toMatchObject({id: expect.any(Number), title: expect.any(String)})
});
...
//💡 TIP: If you want to apply this to all tests, put this assertion as axios extension
  test('When emitting event, ensure that if a response is not aligned with the OpenAPI', async () => {
    // Arrange
    const eventToAdd = getSensorEvent({
      temperature: 80, //💡 TIP: We need high temperature to trigger notification
      notificationCategory: getShortUnique(), //💡 TIP: Unique category will lead to unique notification URL. This helps in overriding the nock
