...
beforeEach(() => {
  nock.cleanAll();
  nock('http://localhost').post('/notification').reply(200, { success: true });
  nock('http://localhost').get('/notification').reply(200, { success: true });
...
describe('Sensors test', () => {
  ...
  test('When temperature is above 50, then the right notification should be sent', async () => {
    ...
    const highTempScope = nock('http://localhost')
      .post(
    ...
    // Assert
    expect(highTempScope.isDone()).toBe(true);
  ...
  test('When emitting a new event and the notification service replies with 500 error, then the added event was still saved successfully', async () => {
    ...
    const fetchResponse = await request(expressApp).get(
      `/sensor-events/${eventToAdd.category}/status`,
    );

    // Assert
    expect(addResponse.status).toBe(200);
    expect(fetchResponse.body).toMatchObject([eventToAdd]);
...
test('When a response is not aligned with the OpenAPI (Swagger)', async () => {
  ...
  expect(addResponse).toSatisfyApiSpec();
});
