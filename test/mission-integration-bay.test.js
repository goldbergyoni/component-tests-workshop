...
describe('Sensors test', () => {
  ...
  test('When emitting a new event and the notification service replies with 500 error, then the added event was still saved successfully', async () => {
    ...
    // Act
    const responseOnPost = await request(expressApp).post('/sensor-events').send(eventToAdd);
    const responseOnGet = await request(expressApp).get(`/sensor-events/${responseOnPost.body.id}`).send();
    ...
    expect(responseOnGet).toMatchObject({
      statusCode: 200,
      body: eventToAdd
    })
  });
  ...
  test('When emitting a new event and the response arrives with some delay, then the added event was still saved successfully', async () => {
    ...
    nock('http://localhost').post(`/notification/${eventToAdd.notificationCategory}`).delay(10000);
