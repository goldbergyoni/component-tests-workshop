...
describe('Sensors test', () => {
  ...
  test('When a new valid event is posted to /sensor-events route, it\'s indeed retrievable from the DB', async () => {
    ...
    const event = (await request(expressApp).get(`/sensor-events/${addedEventId}`)).body;

    expect(response).toMatchObject({ status: 200, body: eventToAdd });
    expect(event).toMatchObject(eventToAdd);
  });
  ...
  test('When an unknown event id is requesting, Then get back null body', async () => {
    // Arrange
    const unknownEventId = -1;

    // Act
    const response = await request(expressApp).get(`/sensor-events/${unknownEventId}`);

    // Assert
    expect(response.body).toBe(null);
  });

  test('When an existed event is deleting, it\'s indeed retrievable from the DB', async () => {
    // Arrange
    const eventToDelete = getSensorEvent();
    const anotherEvent = getSensorEvent();
    const eventToDeleteId = (await request(expressApp).post("/sensor-events").send(eventToDelete)).body.id;
    const anotherEventId = (await request(expressApp).post("/sensor-events").send(anotherEvent)).body.id;

    // Act
    // 💡 TIP: use any http client lib like Axios OR supertest
    // 💡 TIP: This is how it is done with Supertest -> await request(expressApp).post("/sensor-events").send(eventToAdd);
    const response = await request(expressApp).delete(`/sensor-events/${eventToDeleteId}`);

    // Assert
    // 💡 TIP: You may check the body and the status all together with the following syntax:
    // expect(receivedResponse).toMatchObject({status: 200, body: {...}});

    const event = (await request(expressApp).get(`/sensor-events/${anotherEventId}`)).body;

    expect(response.status).toBe(200);
    expect(event).toMatchObject(anotherEvent);
  });
