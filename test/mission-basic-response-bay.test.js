...
describe('Sensors test', () => {
  ...
  test('When category is not specified, should get http 400 error', async () => {
    ...
    // Act
    const receivedAPIResponse = await request(expressApp).post("/sensor-events").send(eventToAdd);
  ...
  test('When inserting a valid event, should get successful response', async () => {
    ...
    expect(receivedAPIResponse).toMatchObject({status: 200, body: {...eventToAdd}});
  });

  // ✅ TASK: Test that when a new valid event is posted to /sensor-events route, it's indeed retrievable from the DB
  // 💡 TIP: In to assert phase, query to get the event that was added
  // 💡 TIP: Whenever possible, use the public API for verification (not direct DB access)
  test('Test that when a new valid event is posted to /sensor-events route, it\'s indeed retrievable from the DB', async () => {
    // Arrange
    const eventToAdd = {
      temperature: 20,
      color: 'Green',
    ...
    const eventId = addAPIResponse.body.id;
    // Act
    const getAPIResponse = await request(expressApp).get(`/sensor-events/${eventId}`).send(eventToAdd);
    // Assert
    expect(getAPIResponse).toMatchObject({status: 200, body: {...eventToAdd}});
