...
describe('Sensors test', () => {
  ...
  test('When inserting a valid event, should get successful response', async () => {
    ...
    expect(receivedResponse).toMatchObject({ status: 200, _body: eventToAdd})
  });
  ...
  test('When inserting a valid event, new event should be accessible via API', async () => {
    const eventToAdd = getSensorEvent()

    const response = await request(expressApp).post("/sensor-events").send(eventToAdd);

    const receivedResponse = await request(expressApp).get(`/sensor-events/${response._body.id}`)
  ...
  test('When an internal unknown error occurs during request, Then get back 500 error', async () => {
    ...
    sinon.stub(SensorsService.prototype, 'addEvent').rejects(new Error("something went wrong"))
