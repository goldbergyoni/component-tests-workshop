...
describe('Sensors test', () => {
    ...
    test('When inserting a valid event, should be able to retrieve it from the DB', async () => {
        // Arrange
        const eventToAdd = {
            temperature: 20,
            color: 'Green',
        ...
        // Act
        const receivedResponse = await request(expressApp).post("/sensor-events").send(eventToAdd);
        const receivedEvent = await request(expressApp).get(`/sensor-events/${receivedResponse.body.id}`);

        // Assert
        expect(receivedEvent).toMatchObject({status: 200,
            body: {
                "id": expect.any(Number),
                "temperature": 20,
                "color": "Green",
