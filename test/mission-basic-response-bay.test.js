...
describe('Sensors test', () => {
  ...
  test('When inserting a valid event, it can be retrieved', async () => {
    // Arrange
    const eventToAdd = {
      temperature: 60,
      color: 'Yellow',
      weight: 12,
      status: 'active',
      category: 'Main bedroom',
    };
    // Act
    // 💡 TIP: use any http client lib like Axios OR supertest
    // 💡 TIP: This is how it is done with Supertest -> await request(expressApp).post("/sensor-events").send(eventToAdd);
    let receivedResponseForAddingEvent = await request(expressApp).post("/sensor-events").send(eventToAdd);
    let eventId = receivedResponseForAddingEvent.body.id;
    let receivedResponseQueryEvent = await request(expressApp).get(`/sensor-events/${eventId}`);//.send();
    // Assert
    // 💡 TIP: You may check the body and the status all together with the following syntax:
    // expect(receivedResponse).toMatchObject({status: 200, body: {...}});
    expect(receivedResponseQueryEvent).toMatchObject({
      status: 200,
      body: {
        id: eventId,
        temperature: 60,
  ...
  test('When an internal unknown error occurs during request, Then get back 500 error', async () => {
    ...
    sinon.stub(SensorsService.prototype, 'addEvent').rejects(new Error("Can not add new event to data access"));
