...
describe('Sensors test', () => {
  ...
  test('When category is not specified, should get http 400 error', async () => {
    ...
    // Assert
    expect(receivedResponse).toMatchObject({ status: 400 });
  ...
  test('When inserting a valid event, should persist to the database', async () => {
    // Arrange
    const eventToAdd = getSensorEvent();
    ...
    expect(receivedResponse).toMatchObject({status: 200, body: { id, ...eventToAdd }});
  });
  ...
  test('When an internal unknown error occurs during request, Then get back 500 error', async () => {
    ...
    sinon.stub(SensorsEventService.prototype, 'addEvent').rejects(new Error("Failed to add event"));
    const eventToAdd = getSensorEvent();
