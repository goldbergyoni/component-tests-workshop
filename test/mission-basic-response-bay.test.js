...
describe('Sensors test', () => {
  ...
  test('When inserting a valid event, should get successful response', async () => {
    // Arrange
    const eventToAdd = getSensorEvent({category: "test"})
    // Act
    // 💡 TIP: use any http client lib like Axios OR supertest
    // 💡 TIP: This is how it is done with Supertest -> await request(expressApp).post("/sensor-events").send(eventToAdd);
    const receivedResponse = await request(expressApp).post('/sensor-events').send(eventToAdd);
  ...
  test('When a new valid event is posted to /sensor-events route, its indeed retrievable from the DB', async () => {
    // Arrange
  ...
  test('When an internal unknown error occurs during request, Then get back 500 error', async () => {
    // Arrange
    const eventToAdd = getSensorEvent()
    const spyOnAddEvent = sinon.stub(SensorsService.prototype, 'addEvent')
    spyOnAddEvent.rejects(new Error("Error explanation"))
    ...
    // Assert
    expect(createdEvent.status).toBe(500)
    spyOnAddEvent.restore()
  });
