...
describe('Sensors test', () => {
  ...
  test('When inserting a valid event, should get successful response', async () => {
    ...
    const body = receivedResponse.body;

    // Assert
    // 💡 TIP: You may check the body and the status all together with the following syntax:
    expect({status: receivedResponse.status, body}).toMatchObject({
      status: 200,
      body: {
        category: 'something'
      }
  ...
  test('When inserting a valid event, should be able to retrieve it', async () => {
    ...
    // 💡 TIP: This is how it is done with Supertest -> await request(expressApp).post("/sensor-events").send(eventToAdd);
    const body = receivedResponse.body;
    const eventResponse = await request(expressApp).get(`/sensor-events/${body.id}`);
    const eventBody = eventResponse.body;

    // Assert
    // 💡 TIP: You may check the body and the status all together with the following syntax:
    expect({status: eventResponse.status, body: eventBody}).toMatchObject({
      status: 200,
      body: {
        category: 'something'
      }
