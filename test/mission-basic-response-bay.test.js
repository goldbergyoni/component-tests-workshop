...
describe('Sensors test', () => {
  ...
  test('when a new valid event is posted to /sensor-events route, it is indeed retrievable from the DB', async () => {
    ...
    // 💡 TIP: This is how it is done with Supertest -> await request(expressApp).post("/sensor-events").send(eventToAdd);
    const eventToAdd = getSensorEvent({});
    const createEventResponse = await request(expressApp)
      .post('/sensor-events')
      .send(eventToAdd);

    const getCreatedEventResponse = await request(expressApp)
      .get(`/sensor-events/${createEventResponse.body.id}`)
      .send(eventToAdd);
    ...
    expect(getCreatedEventResponse).toMatchObject({
      status: 200,
      body: eventToAdd,
    });
  });
