...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    ...
    eventId = receivedResponse.body.id;
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    ...
    // Assert
    expect(receivedResponse1).toMatchObject({
        status: 200,
        body: eventToAdd,
    });
  ...
  // 💡 TIP: At first, query for the event that was added in the first test (In the first test above, store
        ...
        // Act
        const retrieveResponse = await request(expressApp).get(`/sensor-events/${eventId}`);
  ...
  test('When adding a invalid event,  the event is NOT saved to the DB', async () => {
    // Arrange
    const eventToAdd = getSensorEvent({category: undefined})

      // Act
    receivedResponse = await request(expressApp).post("/sensor-events").send(eventToAdd);
    ...
    await request(expressApp).get(`/sensor-events/undefined/status`)
    .expect(res => {
