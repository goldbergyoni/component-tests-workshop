...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    ...
    const matchingRecord = await request(expressApp).get(
        `/sensor-events/${response.body.id}`
    );
    expect(matchingRecord).toMatchObject({status: 200, body: {
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    ...
    const firstReceivedResponse = await request(expressApp).post("/sensor-events").send(eventTOAdd);
    const secondReceivedResponse = await request(expressApp).post("/sensor-events").send(eventTOAdd)

    expect(firstReceivedResponse.status).toEqual(200);
    expect(secondReceivedResponse.status).toEqual(409);
  });
  ...
  test("test that when an event is posted without temp it doenst not successed and not saved in the db", async () => {
      ...
      expect(record).toMatchObject({status:200, });
