...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    ...
    const eventToAdd = {
      ...
      temperature: 20,
      reason: `Thermostat-${getTimestamp()}`, // This must be unique
      color: 'Green',
      weight: 80,
      status: 'active',
    };
  ...
  function getTimestamp() {
    ...
    return `${date.getTime()}-${randomNumber}`;
  }
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    // Arrange
    const eventToAdd = getSensorEvent({ reason: `Thermostat-${getTimestamp()}` });
  ...
  test('When querying for event by id, Then the right event is being returned', async () => {
    ...
    const eventToAddId = (await request(expressApp).post("/sensor-events").send(eventToAdd)).body.id;

    // Act
    const response = await request(expressApp).get(`/sensor-events/${eventToAddId}`);
  ...
  test('When a new event is posted to /sensor-events route, the temperature is not specified, Then the event is NOT saved to the DB', async () => {
    // Arrange
    ...
    const events = await request(expressApp).get("/sensor-events");
    expect(events.body.find(({ reason }) => reason === eventToAdd.reason)).toBe(undefined);
  });
  ...
  test('When adding multiple events, then all of them appear in the result', async () => {
    ...
    // Assert
    expect(events.body.find(({ reason }) => reason === eventToAdd1.reason)).toMatchObject(eventToAdd1);
    expect(events.body.find(({ reason }) => reason === eventToAdd2.reason)).toMatchObject(eventToAdd2);
  });
  ...
  test('When querying for a non-existing event, then get http status 404', async () => {
    // Arrange
    const unknownId = 1111111;

    // Act
    const response = await request(expressApp).get(`/sensor-events/${unknownId}`);
  ...
  test('When adding two events at the same time, then both are saved successfully', async () => {
    // Arrange
    const eventToAdd1 = getSensorEvent();
    const eventToAdd2 = getSensorEvent();
    ...
    // Assert
    expect(responses.every(({ status }) => status === 200)).toBeTrue;
  });
  ...
  // 💡 TIP: Jest has a dedicated matcher for unknown values, read about:
  //  https://jestjs.io/docs/en/expect#expectanyconstructor
  test('When adding a valid event, then createdAt, updatedAt, id fields are not null and have the right schema', async () => {
    // Arrange
    const eventToAdd = getSensorEvent();
  ...
  test('When querying for /sensor-events route (i.e. get all) and sorting by the field \'temperature\', then the results are indeed sorted', async () => {
    // Arrange
    const category = 'example';
    const sortBy = 'temperature';
    const eventToAdd1 = getSensorEvent({ category, temperature: 100 });
    const eventToAdd2 = getSensorEvent({ category, temperature: 50 });
    await request(expressApp).post("/sensor-events").send(eventToAdd1);
    await request(expressApp).post("/sensor-events").send(eventToAdd2);

    // Act
    const events = await request(expressApp).get(`/sensor-events/${category}/${sortBy}`);

    // Assert
    const event1 = events.body.find(({ reason }) => reason === eventToAdd1.reason);
    const event2 = events.body.find(({ reason }) => reason === eventToAdd2.reason);
    expect({ event1, event2, result: event1.temperature > event2.temperature }).toMatchObject({
      event1: expect.any(Object),
      event2: expect.any(Object),
      result: true,
    })
  });
  ...
  test('When a sensor event is deleted, then the code is not mistakenly deleting data that was not supposed to be deleted', async () => {
    // Arrange
    const eventToAdd1 = getSensorEvent();
    const eventToAdd2 = getSensorEvent();
    const eventToAddId1 = (await request(expressApp).post("/sensor-events").send(eventToAdd1)).body.id;
    const eventToAddId2 = (await request(expressApp).post("/sensor-events").send(eventToAdd2)).body.id;

    // Act
    let deleteResponse = await request(expressApp).delete(`/sensor-events/${eventToAddId1}`);

    // Assert
    expect(deleteResponse.status).toBe(200);

    const response = await request(expressApp).get(`/sensor-events/${eventToAddId2}`);
    expect(response).toMatchObject({
      status: 200,
      body: eventToAdd2,
    })
