...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    ...
    // expect(receivedResponse).toMatchObject({ status: 200, body: { id:"s" } });
    expect(receivedResponse.status).toBe(200);
    expect(receivedResponse.body).toMatchObject({ ...eventToAdd, id: expect.any(Number) });
  ...
  test('When a record exist with a specific reason and trying to add a second one, then it fails with status 409', async () => {
    const eventToAdd = getSensorEvent({reason: "test"})
  ...
  test('When querying for event by id, Then the right event is being returned', async() => {
    ...
    const eventToAdd = getSensorEvent()
    const { body: createdEvent } = await request(expressApp).post('/sensor-events').send(eventToAdd);
    const { body: event } = await request(expressApp).get('/sensor-events/' + createdEvent.id);
  ...
  test('When a new event is posted to /sensor-events route, the temperature is not specified -> the event is NOT saved to the DB!', async() => {
    ...
    const createdEvent = await request(expressApp).post('/sensor-events').send(eventToAdd);
    expect(createdEvent.status).toBe(400);
    expect(createdEvent.body).toBeEmpty();
  });
  ...
  test('When an event is deleted, then its indeed not existing anymore', async() => {
    const eventToAdd = getSensorEvent()
    const createdEventResponse = await request(expressApp).post('/sensor-events').send(eventToAdd);
    const deletedEventResponse = await request(expressApp).delete('/sensor-events/' + createdEventResponse.body.id);
    const eventResponse = await request(expressApp).get('/sensor-events/' + createdEventResponse.body.id);
    expect(deletedEventResponse.status).toBe(200);
    expect(eventResponse.body).toBe(null);
  });
  ...
  test('When adding multiple events, then all of them appear in the result', async () => {
    const { body: event1 } = await request(expressApp).post('/sensor-events').send(getSensorEvent({category: "test"}));
    const { body: event2 } = await request(expressApp).post('/sensor-events').send(getSensorEvent({category: "test"}));
    const { body: event3 } = await request(expressApp).post('/sensor-events').send(getSensorEvent({category: "test"}));
    const eventsResponse = await request(expressApp).get('/sensor-events/test/category');
    expect(eventsResponse.body).toIncludeAllMembers([event1, event2, event3])
  });
  ...
  //   try {
  //     const eventsResponse = await request(expressApp).get('/sensor-events/not-exist');
  //   } catch (e) {
  //     expect(e).toBe(404)
  //   }
  // });
