...
describe('Sensors test', () => {
  ...
  test('When adding a valid event, Then should get successful confirmation', async () => {
    ...
    expect(response).toMatchObject({
      status: 200,
      body: expect.objectContaining({...eventToAdd, id: expect.any(Number)})
    })
  });
  ...
  test('When querying for event by id, Then the right event is being returned', async () => {
    ...
    // Act
    const response = await request(expressApp).get(`/sensor-events/${res.body.id}`);

    // expect(response.status).toBe(200);
    expect(response).toMatchObject({
      status: 200,
      body: expect.objectContaining({...eventToAdd, id: res.body.id})
    })
  });
  ...
  test('When a new event is posted without a temperature, the event should not be saved to the DB', async () => {
    // Arrange
    const eventToAdd = getSensorEvent();
    ...
    // Assert
    expect(postResponse.status).toBe(400); // Bad request status

    // 💡 TIP: In the assert phase, query to get the event that was (not) added - Ensure the response is empty
    const getResponse = await request(expressApp).get(`/sensor-events/${eventToAdd.category}/reason`);
    expect(getResponse.body).toHaveLength(0); // Not found status
  });
  ...
  test('When an event is deleted, it should not exist anymore', async () => {
    ...
    // Act
    const deleteResponse = await request(expressApp).delete(`/sensor-events/${eventId}`);

    // Assert
    expect(deleteResponse.status).toBe(200); // Deleted successfully
