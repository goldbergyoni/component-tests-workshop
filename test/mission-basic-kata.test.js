...
describe('Sensors test', () => {
  ...
  test('When a new valid event is posted, Then its indeed retrievable from the DB', async () => {
    // Arrange
    const eventToAdd = {
      temperature: 20,
      color: 'Green',
    ...
    const receivedResponse = await request(expressApp).get("/sensor-events/" + postResponse.body.id).send(eventToAdd);
