...
describe('Sensors test', () => {
  ...
  test('When an internal unknown error occurs during request, Then get back 500 error', async () => {
    ...
    sinon
      ...
      .rejects(new AppError('DB Unreachable', true, 500));
  ...
  test('When an internal error occurs during request, Then the logger writes the right error', async () => {
    ...
    // Act
    const eventToAdd = getSensorEvent();
    const appError = new AppError('DB Unreachable', true, 500);
    sinon.stub(SensorsEventService.prototype, 'addEvent').rejects(appError);
    ...
    expect(spyOnLogger.lastCall.firstArg).toBe(appError);
  });
  ...
  test('When an internal error occurs during request, Then a metric is fired', async () => {
    ...
    const errorName = 'db-is-unaccessible';

    const appError = new AppError(errorName, true, 500);
    sinon.stub(SensorsEventService.prototype, 'addEvent').rejects(appError);

    await request(expressApp).post('/sensor-events').send(eventToAdd);

    console.log('spyOnMetricsExporter', spyOnMetricsExporter.getCalls());

    expect(spyOnMetricsExporter.lastCall.args[1]).toEqual({
      errorName
    });
  ...
  test('When an internal NON-TRUSTED error occurs during request, Then the process exits', async () => {
    ...
    const eventToAdd = getSensorEvent();
    const spyOnMetricsExporter = sinon.spy(metricsExporter, 'fireMetric');
    const errorName = 'DB Unreachable';
